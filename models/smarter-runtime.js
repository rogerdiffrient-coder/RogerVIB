...
(() => {
  const M = window.SMARTER_V06_CONFIG;
  const parts = window.SMARTER_V06_PARTS || [];
  if (!M || !parts.length) {
    console.error('Smarter weights/config are missing');
    return;
  }
  ...
  delete window.SMARTER_V06_PARTS;
  ...
  RogerVIB.registerModel({
    id: 'smarter', name: 'Smarter', order: 60, alpha: true, params: M.params, context: M.context, status: 'alpha', knownIssues: 'experimental; may be slower or say weird things',
    description: `RogerVIB v${M.version || '0.6.0'} Smarter Alpha — ${M.params.toLocaleString()}-parameter, ${M.layers}-layer Transformer with grounded tools, RoPE, and ${M.context}-token context.`,

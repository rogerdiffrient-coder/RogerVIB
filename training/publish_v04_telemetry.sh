...
case "$MODE" in
  started)
    write_status "STARTED" "GitHub runner reached the first telemetry step"
  ...
  quality)
    ...
    write_status "QUALITY_COMPLETE" "Browser-like quality test completed"
    ;;

  quality_failed)
    QUALITY="training/latest_v04_quality_report.txt"
    if [ -f "$QUALITY" ]; then cp "$QUALITY" "$REPORT_DIR/quality-${GITHUB_RUN_ID:-unknown}.txt"; fi
    write_status "QUALITY_FAILED" "Candidate was rejected by browser-like quality checks"
    ;;

  published)
    write_status "PUBLISHED" "Final v0.4 candidate passed validation and quality checks and was published"
    ;;

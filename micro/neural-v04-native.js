...
(() => {
  ...
  function runSelfTest(){
    const test=config?.self_test;
    if(!test)throw new Error('v0.4 config is missing browser self-test data');
    const expectedId=Number(test.context_id);
    const actualId=fnv1a(String(test.context||''),Number(config.hash_buckets)||104000);
    if(actualId!==expectedId)throw new Error(`hash self-test failed: ${actualId} != ${expectedId}`);
    const state=step(actualId,new Float32Array(Number(config.hidden_size)||96));
    const indices=Array.isArray(test.logit_indices)?test.logit_indices:[];
    const values=Array.isArray(test.logit_values)?test.logit_values:[];
    const tolerance=Number(test.tolerance)||0.06;
  ...
  function sample(logits,temp=.62,topK=10,recent=''){
    ...
    const ranked=Array.from(logits,(value,index)=>{let score=Number(value);const ch=vocab[index]||'';if(ch===last)score-=0.28;if(recent.length>=4&&recent.endsWith(ch.repeat(4)))score-=2.0;return {value:score,index};}).filter(x=>Number.isFinite(x.value)).sort((a,b)=>b.value-a.value).slice(0,topK);
    ...
    let r=Math.random()*total;for(let i=0;i<ranked.length;i++){r-=probs[i];if(r<=0)return ranked[i].index;}return ranked[0].index;
  }
  ...
  async function reply(input,history=[]){
    await load();
    const H=Number(config.hidden_size)||96;let hidden=new Float32Array(H);const prime=historyPrompt(input,history).slice(-(Number(config.prime_chars)||360));let seen='',state=null;

// RogerVIB Micro v0.4 — pretrained neural inference, browser-native JS runtime.
...
(() => {
  ...
  function assertFiniteSample(array,name){
    ...
    for(let i=0;i<array.length;i+=stride){
      if(!Number.isFinite(array[i]))throw new Error(`${name} contains invalid numbers`);
    }
  }
  ...
  async function load(){
    ...
    loadPromise=(async()=>{INFO.loading=true;INFO.error='';try{
      ...
      assertFiniteSample(weights.embeddingScales,'embedding scales');
      assertFiniteSample(weights.wih,'GRU input weights');
      assertFiniteSample(weights.whh,'GRU recurrent weights');
      assertFiniteSample(weights.headW,'output weights');
      ...
      weights=null;config=null;INFO.ready=false;INFO.error=errorText(e)||'unknown neural runtime error';console.error('RogerVIB v0.4 native load failed:',e);throw new Error(INFO.error);
    }finally{INFO.loading=false;loadPromise=null;}})();
    return loadPromise;
  }
  ...
  function step(contextId,hidden){
    ...
    for(let i=0;i<H;i++){
      ...
      if(!Number.isFinite(next[i]))throw new Error('v0.4 recurrent state became invalid');
    }
  ...
  function sample(logits,temp=.62,topK=10,recent=''){
    ...
    const ranked=Array.from(logits,(value,index)=>{
      let score=Number(value);
      const ch=vocab[index]||'';
      if(ch===last)score-=0.28;
      if(recent.length>=4&&recent.endsWith(ch.repeat(4)))score-=2.0;
    ...
    const max=ranked[0].value,probs=ranked.map(x=>Math.exp((x.value-max)/Math.max(.05,temp))),total=probs.reduce((a,b)=>a+b,0);
  ...
  function historyPrompt(input,history=[]){
    ...
    const lines=[];
    for(const m of messages){const role=m?.role==='bot'?'roger':'user';lines.push(`${role}: ${sanitize(m?.text).replace(/\n+/g,' ').slice(0,220)}`);}
  ...
  function isLooping(text){
    ...
    if(text.length>=48&&text.slice(-24)===text.slice(-48,-24))return true;
    if(text.length>=30){const tail=text.slice(-30);const unique=new Set(tail).size;if(unique<=3)return true;}
    return false;
  }
  ...
  async function reply(input,history=[]){
    ...
    const prime=historyPrompt(input,history).slice(-(Number(config.prime_chars)||360));
    let seen='',state=null;

    for(let i=0;i<prime.length;i++){
      seen+=prime[i];state=step(hashEndingAt(seen),hidden);hidden=state.hidden;
      if((i&47)===47)await yieldFrame();
    }
    if(!state)state=step(hashEndingAt(' '),hidden);

    let logits=state.logits,answer='',generated=prime;
    const maxChars=Math.min(180,Number(config.max_reply_chars)||180);
    for(let i=0;i<maxChars;i++){
      const id=sample(logits,i<10?.48:.62,i<10?7:11,answer),ch=vocab[id]??'?';
    ...
    if(isLooping(answer))answer=answer.replace(/(.)\1{4,}$/,'$1$1').trim();
    return answer||'(tiny neural silence)';
  }

  window.RogerVIBNeuralV04={info:INFO,load,reply};
  window.ROGERVIB_NEURAL_RUNTIME='native-js-v0.4.1';
})();

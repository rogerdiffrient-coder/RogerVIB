...
(() => {
  ...
  function currentMembers(){
    return users().map(u=>({id:u.id,name:u.name,sub:u.model,avatarText:avatarText(u),color:avatarColor(u)}));
  }
  ...
  function renderSetup(chat){
    ...
    if(!available.length){
      ...
      setupPage.querySelector('[data-create]').disabled=true;
    } else {
      for(const member of available)membersBox.append(memberRow(member,selected.has(member.id)));
    }
    setupPage.querySelector('[data-create]').onclick=()=>{
  ...
  function memberNames(ids){const list=users();return (ids||[]).map(id=>list.find(u=>u.id===id)?.name).filter(Boolean);}
  function decorateChatList(){
    const list=document.getElementById('chatList');if(!list)return;
    const all=chats(),meta=metas();const rows=[...list.querySelectorAll('.chat-entry')];
    rows.forEach((row,i)=>{
      const chat=all[i];if(!chat)return;
      row.querySelector('.rv-chat-member-sub')?.remove();
      const names=memberNames(meta[chat.id]?.members);if(!names.length)return;
      const sub=document.createElement('div');sub.className='rv-chat-member-sub';sub.textContent=names.join(', ');
      row.querySelector('.chat-item')?.append(sub);
    });
  }

  function bootSetup(){
    // main.js creates a placeholder chat on an empty install. Claim it immediately so
    // users never land on the old single-bot empty screen first.
    maybeRender();
    setTimeout(maybeRender,0);
    setTimeout(maybeRender,50);
    setTimeout(maybeRender,200);
    setTimeout(maybeRender,600);
  }
  ...
  window.addEventListener('DOMContentLoaded',()=>{
    ...
    const observer=new MutationObserver(()=>{
      if(!document.querySelector('.rv-users-main-page')){
        ...
        if(chat&&(chat.messages||[]).length===0&&!metas()[chat.id]?.setupDone&&!setupPage)setTimeout(maybeRender,0);
      }
    });

...
(() => {
  ...
  function editUserModal(existing){
    ...
    modal.querySelector('[data-save]').onclick=()=>{
      ...
      const list=users();
      if(existing){const u=list.find(x=>x.id===existing.id);if(u){u.name=n;u.model=m;u.prompt=p;}}
      else list.push({id:uid(),name:n,model:m,prompt:p});
      write(USERS_KEY,list);modal.remove();render();
    };
  ...
  function renderUsersMain(){
    ...
    hero.innerHTML='<div><div class="rv-users-eyebrow">AI USERS</div><h1>Your AI users</h1><p>Create separate AI identities with their own model and personality.</p></div>';
    ...
    const list=users();
    if(!list.length){
      const empty=document.createElement('div');empty.className='rv-main-users-empty';empty.innerHTML='<div class="rv-main-users-empty-icon">R</div><h2>No AI users yet</h2><p>Make one and give it a model and personality.</p>';grid.append(empty);
  ...
  window.addEventListener('DOMContentLoaded',()=>{
    document.querySelector('.rv-social-backdrop')?.remove();
    ensureTabs();render();
    const observer=new MutationObserver(()=>document.querySelector('.rv-social-backdrop')?.remove());
